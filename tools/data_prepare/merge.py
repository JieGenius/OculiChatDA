...
def main():
    ...
    with open(med_dialog_path, "r") as f:
        data = json.load(f)
    for item in data:
        tmp ={"conversation": []}
        assert len(item) % 2 == 0
    ...
    #
    # med_conversation_num = sum(counter_med_dialog.values()) + sum(counter.values())
    # line_num = sum(1 for line in open(universal_corpus_path, "r"))
    # universal_corpus_num = min(9 * med_conversation_num, 500000, line_num)

...
def parse_document(doc_path):
    ...
    for paragraph in doc.paragraphs:
        ...
        if paragraph.text.strip().endswith(
                ':') or paragraph.text.strip().endswith('?'):
            # New question found
            if current_question:
                data[current_idx] = current_question
            current_idx, question = paragraph.text.strip().split(
                ')', maxsplit=1)
...
def parse_answers(doc_path):
    ...
    for paragraph in doc.paragraphs:
        if paragraph.text.strip().startswith('Answers'):
            ...
            question_number, answer = paragraph.text.strip().split(
                '.', maxsplit=1)
...
def generate_llm_data(formatted_data):
    ...
    for item in formatted_data:
        ...
        data.append({
            'conversation': [{
                'system':
                '你是一名眼科专家，你需要解答患者的疑问，提供准确的回答，必要时，提醒患者及时挂号就医。',
                'input':
                question + '\n' + '\n'.join([
